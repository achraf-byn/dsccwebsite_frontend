  ...
  return <AnimatePresence>
    {showIntro && <motion.div className="cloud-intro" role="status" aria-label="Initializing DSCC Cloud" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: prefersReducedMotion ? .18 : .14 }}>
      ...
      <div className="cloud-intro-stage">
        <motion.div className="cloud-intro-kicker" initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .35, delay: prefersReducedMotion ? 0 : .12 }}><T>DSCC / CLOUD SYSTEM </T><span><T>INIT</T></span></motion.div>
        ...
        <motion.p className="cloud-intro-status" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .3, delay: prefersReducedMotion ? 0 : .55 }}><motion.span animate={prefersReducedMotion ? { opacity: 1 } : { opacity: [1, 1, 0] }} transition={{ duration: 2, times: [0, .99, 1], delay: .05 }}><T>Initializing DSCC Cloud...</T></motion.span>{!prefersReducedMotion && <motion.span className="cloud-intro-ready-text" initial={{ opacity: 0 }} animate={{ opacity: [0, 0, 1] }} transition={{ duration: 2, times: [0, .99, 1], delay: .05 }}><T>DSCC Cloud Ready</T></motion.span>}</motion.p>
        ...
        <motion.div className="cloud-intro-mark" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .35, delay: prefersReducedMotion ? .15 : 2.1 }}>
          <strong><T>DSCC</T></strong><span><T>Data Science &amp; Cloud Computing Club</T></span>
        </motion.div>
