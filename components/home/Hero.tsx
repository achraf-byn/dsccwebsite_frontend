...
function FeatureStrip() {
  ...
  return <div className="hero-features" ref={ref} aria-label="What DSCC helps students do">
    {features.map(({ title, description, icon: Icon }, index) => <motion.div className="hero-feature" key={title} initial={{ opacity: 0, y: 18 }} animate={isInView ? { opacity: 1, y: 0 } : undefined} transition={{ ...entrance, delay: prefersReducedMotion ? 0 : index * .08 }}>
    ...
    <div className="hero-container">
      <div className="hero-copy">
        ...
        <motion.div className="hero-actions" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ ...entrance, delay: prefersReducedMotion ? 0 : .56 }}>
          <a className="hero-button hero-button-primary" href="/contact">Join the Club <ArrowRight size={17} /></a>
          <a className="hero-button hero-button-secondary" href="/events">Explore Events <ArrowUpRight size={16} /></a>
        </motion.div>
      ...
      <div className="hero-artwork-wrap" aria-label="DSCC students collaborating on a laptop">
        ...
        <motion.div className="hero-artwork" initial={{ opacity: 0, x: 34 }} animate={{ opacity: 1, x: 0 }} transition={{ ...entrance, delay: .18 }} style={canParallax && !prefersReducedMotion ? { x: artworkSpringX, y: artworkSpringY } : undefined}>
