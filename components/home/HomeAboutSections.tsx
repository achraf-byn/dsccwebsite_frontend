...
import { ArrowRight, BrainCircuit, Cloud, Code2, Database, Network, Sparkles } from 'lucide-react'
...
export default function HomeAboutSections() {
  ...
  return <section className="home-about-sections" ref={ref} aria-label="About DSCC"><div className="home-about-shell">
    ...
    <motion.div className="home-field-map" {...reveal(.2)}><div className="home-field-lines" aria-hidden="true" /><div className="home-field-core"><Sparkles size={17} /><strong>DSCC</strong><small>ECOSYSTEM</small></div>{fields.map((field, index) => <span className={`home-field-pill home-field-pill-${index + 1}`} key={field}>{field}</span>)}<div className="home-field-chip home-field-chip-ai"><BrainCircuit size={13} />AI</div><div className="home-field-chip home-field-chip-cloud"><Cloud size={13} />Cloud</div><div className="home-field-chip home-field-chip-data"><Database size={13} />Data</div><div className="home-field-chip home-field-chip-code"><Code2 size={13} />Build</div><Network className="home-field-network-icon" size={17} /></motion.div>
  </div></section>
}
